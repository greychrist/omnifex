...
Per-session AI summaries on the project tab — opt-in per account, generated by the model of your choice when each session's tab closes.

Installers remain **unsigned**.

### Added

- **Per-session AI summaries** on the project tab. Each session row now shows a one-line headline (with a chevron-expandable paragraph) generated by the model you select. A `↻` button on each row regenerates on demand; it disables itself when the JSONL file size hasn't changed since the last successful summary (no API spend on no-op clicks). Auto-on-close re-runs every time you close a tab so resumed work picks up new content.
- **Sidecar JSON files** (`<session-uuid>.summary.json`) cache each summary next to the JSONL. Atomic writes (tmp + rename), schema-versioned for forward-compat, survives reinstalls.
- **Per-account opt-in** in Account Settings — a new toggle ("Generate summaries when sessions close") plus a Summary-model picker. Cost estimate shown inline so you know what to expect (Haiku 4.5 ≈ $0.005–$0.05/session, Sonnet 4.6 ≈ $0.015–$0.15/session, Opus 4.7 ≈ $0.075–$0.75/session). Pro/Max users: pulls from your plan allotment, not billed per-token.
- New IPC channels: `summary_get`, `summary_generate`, `update_account_summary`. New event channel: `session-summary:updated` (broadcasts when a new sidecar is written).

### Changed

- **`SessionList.tsx` row layout** — the "First message" column is now "Summary" and replaces the truncated first-user-message preview with the AI-generated headline when a sidecar exists. Falls back to the existing first-message preview when no summary is on disk yet.
