...
- [x] **2.2** `query.getContextUsage()` — the existing client-side widget in `SessionHeader.tsx` (which computed `totalTokens / hardcoded_limit_for_model`) was swapped for authoritative numbers from the SDK. `ClaudeCodeSession.tsx` calls `api.sessionContextUsage(tabId)` twice per turn: once on `system:init` (baseline) and again on each `result` message (refresh). Tooltip now shows the per-category breakdown (messages, system prompt, tools, memory, MCP, etc.) when SDK data is present, with a `source:` footer distinguishing SDK numbers from the client-side fallback. Old approximation still kicks in if `getContextUsage()` errors out — no regression.
