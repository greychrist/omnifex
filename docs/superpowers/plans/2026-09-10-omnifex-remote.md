...
- [x] 3.5 Electron main spawns/attaches the daemon (`electron/remote-launcher.ts`: probe `/healthz` → spawn DETACHED → poll ≤10s → url or null), serves `remote:url` + `notify:show`; preload publishes the bridge as `__omnifexNative` **only** and `bootstrap.ts` defines `window.electronAPI` in every mode; `main.tsx` awaits `installRemoteBridge()` before booting React.
  - **Found on first real launch (2026-09-10, Greg):** the preload originally exposed the bridge as `electronAPI` too. `contextBridge.exposeInMainWorld` defines the property ReadOnly | DontDelete, so the shim assignment in `bootstrap.ts` threw `TypeError: Cannot assign to read only property 'electronAPI'` inside the handshake try/catch, and every launch logged "daemon handshake failed" and ran legacy IPC — the daemon saw connect + disconnect 3 ms apart. Fixed by making the preload publish `__omnifexNative` only; guarded by `electron/__tests__/preload-exposure.test.ts` and `src/lib/remote/__tests__/bootstrap.test.ts`. The headless verification never hit this because the web path has no preload.
...
2. **Try the app**: `npm start`. `remote:url` will probe → spawn the daemon detached → the renderer connects and logs `[remote] mode=electron-remote` in devtools. If anything is off, `OMNIFEX_REMOTE=0 npm start` is the old app, untouched. First attempt fell back to legacy (read-only `electronAPI`, see 3.5) — fixed the same day; a session's CLI should now have the daemon as its parent and `~/.omnifex/sessions/` should fill.
...
**Daemon version drift — handled.** The daemon outlives the app, so it outlives an upgrade. `remote-launcher` now compares `/healthz` `version` with `app.getVersion()`: same → attach; different and `sessions.inFlight === 0` → stop (SIGTERM via pid file; `launchctl bootout` first when the LaunchAgent is installed, so KeepAlive cannot resurrect the old build), wait for it to go, spawn from this bundle (re-installing the plist for this bundle's script when launchd owns it), wait for it to answer; different and busy → attach to the old one and check back every 30 s until idle. The app that launches wins, in both directions. `electron/remote/daemon-control.ts` holds the launchd-aware stop/spawn. Verified live: a daemon faked to report 0.4.100 on port 47701 was replaced by 0.4.156 on app launch; the renderer's welcome carried the new version.

**In-app updater vs the daemon (2026-09-10).** The installer's idle gate reads renderer-reported busy tabs (`tab_status_*` stays native under the shim, so that still works), and now also the daemon's `/healthz` `sessions.inFlight`, so a turn running on another device blocks the swap like a local one. `executeInstall` stops the daemon (best-effort, 5 s cap) before spawning the helper, because the helper `rm -rf`s the bundle the daemon runs from; the relaunched app spawns a new daemon. Simulated end to end with two packaged builds: a 0.4.156 daemon (no `inFlight` field, reads as idle) was replaced by a launched 0.4.157 app; the renderer's welcome carried 0.4.157 and the badge went green. Note: `omnifex --version` launches the GUI — Electron does not handle that flag — so read the version out of `app.asar/package.json` instead.

**Title-bar Daemon button (2026-09-10).** `src/components/DaemonStatusPopover.tsx`, between Cost and Sessions: green check connected, amber spinner between sockets, red cross when there is no daemon (legacy IPC) or the socket is down. The popover reads `/healthz` every 5 s while open — version (flagged against the app's), protocol, address, clients, sessions live/running, uptime — with a Refresh button; in legacy mode it explains why there is no daemon. `/healthz` and the other JSON routes now send `access-control-allow-origin: *` because the Electron renderer reads it cross-origin. Health also carries `build` (daemon script mtime) and the launcher compares it in dev only (`app.isPackaged` → skipped), so `npm start` replaces a same-version daemon left over from the previous build.
...
- Daemon shutdown: the "2 orphaned CLI children" seen after the last `omnifex-server stop` was a false positive — the grep matched this session's own shell command text; the processes listed were the desktop app's live sessions (ppid = Electron main). Every check right after a stop found zero orphans. Still worth doing: have `daemon.close()` await engine exits with a SIGKILL deadline rather than exiting right after SIGTERM.
- Deep link (`/#session=<id>`) to a session whose process is gone opens its tab on the New Session form (prefilled); resuming takes a tap on Start Session. Auto-resume on deep link is a small follow-up in the shim (`session_rebind` → `session.resume` for a tab created from a link).
