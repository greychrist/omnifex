...
- **Daemon shutdown can leave CLI children alive.** After `omnifex-server stop` two `claude` processes from throwaway sessions were still running (killed by hand). `lifecycle.stop()` sends SIGTERM through `engine.close()` and the daemon exits without waiting; the CLI evidently survives that while idle. Fix candidates: await the engines' exit (with a SIGKILL deadline) in `daemon.close()`, and/or spawn children in their own process group and signal the group. The `session.resume` path already copes with a dead child, and a survivor is harmless but wasteful.
