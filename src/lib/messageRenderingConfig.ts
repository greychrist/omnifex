...
export const DEFAULT_KINDS: MessageKindConfig[] = [
  ...
  {
    // Historical companion to `permission.askUserQuestion`. The permission
    // kind is the *live* prompt (question awaiting an answer); this one is
    // the persisted record of the resolved interaction — the tool_use plus
    // its matching tool_result rendered as a single compact card in
    // scrollback. Same purple accent so the two read as variants of the
    // same conversation type, but a distinct id so users can theme or
    // compact-hide them independently.
    id: "tool.askUserQuestion.answered",
    label: "Agent question (answered)",
    description: "Persisted record of an answered AskUserQuestion interaction (one line per question with the user's pick).",
    origin: "system",
    icon: "CircleHelp",
    headerLabel: null,
    accentColor: "purple",
    ...
    hiddenInCompact: false,
    compactBoundaryLocked: false,
    widget: "AnsweredAskUserQuestionCard",
  },
  {
    // Companion to `tool.askUserQuestion.answered`. The user-role message
    // carrying only the matching tool_result has its data folded into the
    // assistant-side answered card; rendering it as its own bubble would
    // just be a redundant JSON-blob row. The renderer returns null for
    // this kind unconditionally — the only reason it exists as a kind
    // (rather than a `return null` heuristic in StreamMessage) is so
    // compactGrouping can treat the message as fully hidden and fold it
    // into surrounding hidden-event runs cleanly.
    id: "tool.askUserQuestion.answered.result",
    label: "Agent question (answered) — tool_result",
    description: "Hidden tool_result paired with the answered AskUserQuestion card. Never rendered standalone.",
    origin: "system",
    icon: "none",
    headerLabel: null,
    accentColor: "purple",
    ...
    hiddenInCompact: true,
    compactBoundaryLocked: false,
    // No widget — the renderer returns null for this kind.
  },
  {
