  ...
  try {
    ...
    if (message.kind === "assistant" && message.raw.message) {
      ...
      const lastCardIdx = (() => {
        ...
        for (let i = visibleBlocks.length - 1; i >= 0; i--) {
          ...
          const presentation = blockKind ? resolveMessageStyle(renderConfig, message, blockKind).presentation : 'card';
    ...
    if (message.kind === "user") {
      ...
      if (isSystemContextText(contentStr)) {
        const cfgHeader = resolveMessageStyle(renderConfig, message, "user.systemContext").headerLabel ?? null;
