...
- Live controls, changeable mid-session, read out on the session status bar: the **model and reasoning effort** (Auto, the model's own default, or low → max) as one control, and the **permission mode** (default / acceptEdits / bypassPermissions / plan). bypassPermissions still stops for the checks Claude Code refuses to skip, such as a recursive delete of the working directory. Models are named by version — Opus 5.5, not Opus — because Claude Code publishes one entry per family and moves which model it resolves to underneath; the picker leads with the newest version of each family, and a **More models** panel holds the older ones — plus versions your account's list leaves out — for pinning a specific one.
...
- **Side chat** — ask about the session without derailing it, Claude Code's `/btw`. A panel beside the transcript takes questions, even while a turn is running, and answers them from the conversation so far, with no tools and nothing added to the session. Type `/btw <question>` in the composer to ask from there. The thread lasts until you close it or the session ends, and shows up on the iPad too.
...
- Image attachments, in-session find, permission prompts, MCP elicitation prompts (a form built from the server's questions, or a browser sign-in), per-tab context-usage / cost readouts, and a per-command summary of the files a Bash command changed.
